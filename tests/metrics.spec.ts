...
describe("Check Metrics Ouput", () => {
    describe("E2E", () => {
        ...
        it("Check Header", () => {
            expect(input_data.output_html.header.replace(/\ /g, "").replace(/\n/g, "")).toBe(`
                <br/>
                <tr>
                    <th style="padding:4px;text-align:left">${input.rendering_options.default_title_for_rows}</th>
                    <th style="padding:4px;text-align:left">cpu <i class="fa fa-circle" style="color:white"></i><i class="fa fa-circle" style="color:white"></i></th>
                    <th style="padding:4px;text-align:left">mem <i class="fa fa-circle" style="color:white"></i><i class="fa fa-circle" style="color:white"></i></th>
                </tr>
            `.replace(/\ /g, "").replace(/\n/g, ""));
        });
        it("Check tooltip", () => {
        ...
        it("Check BG Color, Text Color based on threshold", () => {
            expect(input_data.output_html.body.replace(/\ /g, "").replace(/\n/g, "")).toContain(`
                <td style="padding:4px;text-align:left">
                    web_1 <i class="fa fa-circle" style="color:rgba(50, 172, 45, 0.97)"></i>
                </td>
                <td style="padding:4px;background-color:pink;text-align:left;color:purple">
                    <div data-toggle="tooltip" data-html="true" data-placement="auto" title="Series : dev.server_stats.web_1.cpu.usage | Value : 40.0000%" style="padding-left:10px">Bad 40.0000%</div>
                </td>
            `.replace(/\ /g, "").replace(/\n/g, ""))
        });
        it("Check BG Color, Text Color based on overrides", () => {
            expect(input_data.output_html.body.replace(/\ /g, "").replace(/\n/g, "")).toContain(`
                <td style="padding:4px;text-align:left">
                    web_3 <i class="fa fa-circle" style="color:rgba(50, 172, 45, 0.97)"></i>
                </td>
                <td style="padding:4px;background-color:pink;text-align:left;color:purple">
                    <div data-toggle="tooltip" data-html="true" data-placement="auto" title="Series : dev.server_stats.web_3.cpu.usage | Value : 400.0000%" style="padding-left:10px">Bad 400.0000%</div>
                </td>
                <td style="padding:4px;background-color:darkgreen;text-align:left;color:skyblue">
                    <div data-toggle="tooltip" data-html="true" data-placement="auto" title="Series : dev.server_stats.web_3.mem.usage | Value : 420.0000%" style="padding-left:10px">Something really wrong</div>
                </td>
            `.replace(/\ /g, "").replace(/\n/g, ""))
        });
